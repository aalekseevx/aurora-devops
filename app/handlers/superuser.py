...
@dp.message_handler(
    commands=["set_superuser"], commands_prefix="!", is_superuser=True
)
async def cmd_superuser(message: types.Message):
    ...
    if result:
        return await message.answer(
            _(
                "Successful changed is_superuser to "
                "{is_superuser} for user {user}"
            ).format(is_superuser=not remove, user=user_id)
        )
    return await message.answer(
        _(
            "Failed to set is_superuser to" "{is_superuser} for user {user}"
        ).format(is_superuser=not remove, user=user_id)
    )
