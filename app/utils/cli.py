...
def cli():
    ...
    from app.utils import logging_handler
...
def auto_reload_mixin(func):
    ...
    def wrapper(autoreload: bool, *args, **kwargs):
        if autoreload and aiohttp_autoreload:
            logger.warning(
                "Application started in live-reload mode."
...
def webhook():
    ...
    runner.start_webhook(
        webhook_path=config.WEBHOOK_PATH, port=config.BOT_PUBLIC_PORT
    )
